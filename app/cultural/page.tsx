...
import { getCulturalEvents, EventItem } from "@/lib/data/events";

export default function CulturalPage() {
  const [events] = useState<EventItem[]>(() => getCulturalEvents());
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(
        (ev) =>
          ...
          ev.department?.toLowerCase().includes(term) ||
          ev.sessions.some(
            (s) =>
    ...
    // Filter by participation / audience
    if (participationFilter !== "all") {
      result = result.filter((ev) =>
        ev.sessions.some((s) =>
          s.participation
            ?.toLowerCase()
            .includes(participationFilter.toLowerCase()),
        ),
      );
    }

    setFilteredEvents(result);
  }, [searchTerm, participationFilter, events]);
    ...
    <div className="min-h-screen bg-[#F4F9FD] text-[#0B256B] selection:bg-[#00ACE9]/30 selection:text-[#081B4B]">
        ...
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 relative z-10">
          ...
          >
            ...
            <div className="mt-6 flex flex-wrap items-center gap-2.5 sm:gap-3">
              <div className="bg-transparent border border-white/20 rounded-full px-3 py-1.5 text-xs font-mono text-white flex items-center gap-2 shadow-2xs">
      ...
      <section className="bg-[#F4F9FD] py-10 sm:py-14">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6">
          ...
          {/* 3. Cards Grid - Click directly navigates to /cultural/[slug] */}
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col p-5">
                      ...
                      <div className="font-mono flex flex-wrap items-center gap-x-2 text-[10.5px] font-semibold uppercase tracking-[0.12em] text-[#0062A2]">
                        <span>{event.department || "STUDENT WELFARE"}</span>
